class EquipmentFormHandler {
    ...
    displayEquipmentData(data) {
        ...
        data.data.forEach(item => {
            ...
            if (item.field_type === 'file' || item.field_type === 'image') {
                ...
                geoLocationDisplay.textContent = item.value || 'موقعیت جغرافیایی ثبت نشده است.';
            ...
            .then(data => {
                if (data.success) {
                    ...
                    data.data.fields.forEach(field => {
                        ...
                        fieldDiv.classList.add('col-sm-6', 'border-bottom', 'p-2');
                        const label = document.createElement('label');
                        label.textContent = field.field_name === 'equipment_serial' ? 'سریال تجهیز' : field.field_name;
                        label.classList.add('form-label');
    ...
    handleSaveData() {
        ...
        const geoLocationInput = this.formContainer.querySelector('input[name="geo_location"]');
        if (geoLocationInput) {
