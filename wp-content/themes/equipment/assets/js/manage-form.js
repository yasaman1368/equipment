...
class FormManager {
    ...
    cacheModals() {
        this.modals.addForm = new bootstrap.Modal(document.getElementById('modal-add-form'));
        this.modals.addFormName = new bootstrap.Modal(document.getElementById('modal-add-form-name'));
    }

    initEventListeners() {
        // Form name modal
        document.getElementById('save-modal-form-name').addEventListener('click', () => this.handleAddNewForm());
        ...
        // Main buttons
        document.getElementById('show-forms').addEventListener('click', () => this.handleShowForms());
        document.getElementById('form-selector').addEventListener('change', (e) => this.handleFormSelectorChange(e));
        document.getElementById('save-form-btn').addEventListener('click', () => this.handleSaveForm());
    }
    ...
    handleFeatureTypeChange() {
        ...
        if (['select', 'checkbox', 'radio'].includes(featureType)) {
            document.getElementById('selected-input').textContent = 
                featureTypeElement.options[featureTypeElement.selectedIndex].textContent;
    ...
    createSelectField(featureName, numberOfFields) {
        ...
        if (!hasValidOptions) {
            this.showAlert('خطا', 'حداقل یک گزینه برای لیست کشویی وارد کنید.', 'error');
            return null;
        }
    ...
    createCheckboxRadioField(featureName, featureType, numberOfFields) {
        ...
        options.forEach(option => {
            ...
            if (innerText) {
                ...
                const inputuniqcode = `${featureType}-${numberOfFields}-${innerText.replace(/\s+/g, '-')}`;
        ...
        if (!hasValidOptions) {
            this.showAlert('خطا!', 'حداقل یک گزینه برای گزینه رادیویی یا چک باکس وارد کنید.', 'error');
            return null;
        }
    ...
    handleFormSelectorChange(event) {
        ...
        this.fetchData('/wp-admin/admin-ajax.php?action=get_form_fields', { form_id: formId })
            .then(data => {
                if (data.success) {
    ...
    displayFormData(formData) {
        ...
        if (formData.form_locations && Array.isArray(formData.form_locations) && formData.form_locations.length > 0) {
    ...
    handleSaveForm() {
        ...
        this.fetchData('/wp-admin/admin-ajax.php?action=save_form_data', { form_data: JSON.stringify(formData) })
            .then(data => {
                if (data.success) {
                    this.showAlert('موفق!', data.data.message, 'success')
                        .then(() => this.handleShowForms());
                } else {
        ...
        this.showConfirm(
            ...
            if (result.isConfirmed) {
                this.fetchData('/wp-admin/admin-ajax.php?action=remove_form', { form_id: formId })
                    .then(data => {
                        if (data.success) {
                            this.showAlert('حذف شد!', data.data.message, 'success')
                                .then(() => {
